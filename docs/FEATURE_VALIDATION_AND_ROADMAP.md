...
**Passo 2 — em curso.** Objetivo: tornar visível na app o que o passo 1 já produz no banco. Pontos 1 e 3 entregues na v0.29.0 e validados em 08/08/2026 na v0.29.1; ponto 2 entregue na v0.30.0; ponto 4 por fazer. Ordem, mantendo a disciplina de fatias pequenas:
...
2. ~~**Entrada e linguagem da prévia.**~~ **VALIDADA na instalação real em 09/08/2026; entregue na v0.30.0.** A Library substitui `Documentos gerados` por uma seção permanente `Prévia estruturada`, com estado vazio, lista de prévias, selo de só leitura e compositor inline. Motor, perfil, custo por hora e estado da chave aparecem antes do botão; chave ausente bloqueia a ação e nenhuma chamada paga ocorre automaticamente. O painel pós-transcrição oferece `Ver transcript`, `Criar prévia` e `Continuar na Library`; o novo evento aditivo `transcription_saved` leva ao item persistido específico. A explicação das camadas fica visível até a primeira escolha e depois é compactada por estado local. Textos existem em PT/EN/ES. `Salvar/Abrir no Caderno` não foi simulado: começa apenas com a fundação real de `notebooks`.

   **Evidência técnica e visual:** `tsc` + Vite aprovados, 14 testes do worker aprovados, worker reempacotado antes do Tauri, NSIS gerado e v0.30.0 instalada. O harness visual temporário cobriu transcript #23/documento #9, estado vazio, compositor, chave ausente, retorno do leitor, pós-transcrição e 800 × 1000 sem cortes, sobreposições ou truncamentos; o harness foi removido. Nenhuma geração foi acionada. O Computer Use falhou repetidamente com `0x80070003` ao tentar enumerar janelas; a automação foi retomada com sucesso em 09/08/2026 (sem relação com o repositório) e completou a **captura final na instalação real**: transcript #23 aberto na app v0.30.0 instalada, seção `Prévia estruturada` com o cartão do documento #9 ("Onboarding e Orientação da Comunidade Classe A", detalhado, claude_haiku), compositor aberto via `Criar outra prévia` sem clicar em `Gerar prévia`, janela normal e janela estreita (sidebar colapsada, coluna única) sem cortes, sobreposições ou truncamentos. Nenhuma geração paga foi acionada e nenhum dado foi alterado.
