...
**Estado mais recente:** 14 de agosto de 2026 (v0.44.0; Caderno com árvore/coleções, worker persistente, anotações, referências, links, glossário e exportação real em `.zip` (.md/.docx/prompt IA) entregues e majoritariamente confirmados por Leonardo na máquina real; ver secção 12, Registro de atualizações, para o detalhamento por versão)
**Produto:** UpexNote  
...
> **Nota (14/08/2026):** os pontos abaixo descrevem o estado no dia 8 de agosto (v0.30.0). Desde então, entre v0.31.0 e v0.44.0, o Caderno (`notebooks`) deixou de ser apenas arquitetura aprovada: schema, árvore de coleções, worker persistente (`serve`), endpoint composto de abertura de nota, store partilhado entre `NotebooksView`/`LibraryView`, editor, anotações, referências, links, glossário (consulta manual via dictionaryapi.dev) e exportação real em `.zip` (.md/.docx/prompt para IA, com idioma seguindo a transcrição de origem) foram implementados e majoritariamente confirmados por Leonardo na instalação real. Ver secção 12, Registro de atualizações, para o detalhamento por versão; a cache unificada (Fase D) e o colapsar/expandir da árvore do Caderno continuam pendentes.
...
### Registro — 2026-08-14 (iv): idioma da exportação (documento + prompt para IA) segue o idioma da reunião/transcrição de origem (v0.44.0)

**Contexto:** depois de confirmar a v0.43.0, o Leonardo pediu explicitamente que a exportação não fique presa ao português: "sim mais isos tem que refletir da mesma forma que se fosse a reuniao ou gravação so em portugues ou so espanhol ou so ingles entende se a transcrição estiver em ingles o format do docuemnto de edição segue ingles referencias ingles e prompt ingles s efor espanhol mesma coisa" — ou seja, se a transcrição/reunião de origem está em inglês, o `.docx` exportado (cabeçalhos de secção como "Referências"/"Anotações", subtítulo, rodapé) e o prompt para IA devem sair em inglês; se espanhol, em espanhol; senão, português (comportamento atual, mantido como padrão).

**O que mudou:**
- `services/worker/transcription/db.py`: novas funções `_notebook_note_language(note_id, conn=None)` (segue `notebooks.note_sources` até `transcriptions.language` — diretamente por `transcription_id`, ou por `document_id` → `documents.structured_documents.transcription_id`) e `_normalize_notebook_lang(raw)` (mapeia para `"en"`/`"es"`/`"pt"`, padrão `"pt"` se não houver proveniência ou o idioma não for reconhecido — só os 3 idiomas que a própria app suporta em `i18n.ts`). Novo dicionário `_NOTEBOOK_EXPORT_LABELS` com os rótulos de secção (Anotações/Referências/Glossário/Palavras-chave/Notas ligadas/Proveniência/etc.) nos 3 idiomas.
- `notebook_note_export()`: calcula `lang`/`L` a partir do idioma da nota e usa `L[...]` em vez de strings fixas em português para todos os cabeçalhos `##`. Devolve agora também `"language": lang` na resposta.
- `notebook_note_context_package()`: reaproveita `export["language"]` (já calculado por `notebook_note_export()`) e escolhe entre 3 templates completos do prompt para IA (pt/en/es — mesma estrutura da v0.43.0: ordem de leitura, mensagem de abertura com menção ao UpexNote, exemplo de tom, regras factuais). Devolve também `"language": lang`.
- `cli.py`: `cmd_notebook_export` e `cmd_notebook_context_package_create` passam a emitir o campo `"language"` no JSON, para o frontend saber qual idioma usar no `.docx`.
- `App.tsx`: `buildExportDocx(markdown, lang)` passou a receber o idioma e usa um pequeno mapa `NB_DOCX_STRINGS` (pt/en/es) para o subtítulo ("Exportado do UpexNote"/"Exported from UpexNote"/"Exportado desde UpexNote"), o formato de data (`toLocaleDateString` com locale correspondente) e o rodapé ("Página X de Y"/"Page X of Y"). `exportBundle()` passa `exportObj.language` (vindo do `notebook_export`) para `buildExportDocx()`.

**Validação:** `python3 -c "import ast; ast.parse(...)"` limpo em `db.py` e `cli.py`; `npx tsc --noEmit` limpo em `App.tsx`. Não há como validar em runtime (Windows real) nesta sessão — depende do próximo build/teste do Leonardo, incluindo confirmar que `transcriptions.language` está de facto populado para as notas de teste dele (se estiver vazio/nulo, o padrão cai para português, que é o comportamento anterior).

### Registro — 2026-08-14 (iii): indicador de progresso na exportação + prompt para IA reescrito como prompt de verdade (v0.43.0)

**Contexto:** o `.zip` da v0.42.0 funcionou de ponta a ponta pela primeira vez (diálogo apareceu, 3 ficheiros gerados e salvos, o Leonardo confirmou anexando os 3 de volta). Duas coisas para afinar: (1) a geração demorou e não havia nenhum indicador visual do que estava a acontecer, só o botão a dizer "A guardar..."; (2) o ficheiro "prompt para IA" era só um aviso genérico de "não invente" seguido do conteúdo cru — sem instruções de leitura, sem mensagem de abertura pronta, sem sugestões para o utilizador continuar, sem reconhecer o UpexNote pelo nome. O Leonardo foi específico: quer que QUALQUER IA que receba o pacote comece por uma mensagem amigável que mencione o UpexNote (reforço de marca — "a IA apontou o UpexNote e agora sabe o que enviei"), resuma o material, e sugira por onde continuar.

**O que mudou:**
- `App.tsx`/`App.css`/`i18n.ts`: enquanto `exportBundle()` está a correr, aparece uma linha com spinner + `nbExportGenerating` ("A gerar o seu pacote — a reunir o corpo da nota, anotações, referências e o prompt para IA...") abaixo do botão — reaproveita o padrão `.spinner` já usado no resto da app.
- `services/worker/transcription/db.py`, `notebook_note_context_package()`: o `prompt` deixou de ser uma frase de aviso + conteúdo cru e passou a ser um prompt estruturado com: (1) instrução explícita de ordem de leitura (este prompt primeiro, depois o material, mesmo que a IA receba o `.zip` inteiro); (2) instrução para a primeira mensagem da IA ao utilizador confirmar que leu o material, MENCIONAR O UPEXNOTE PELO NOME, e sugerir 2-3 formas concretas de continuar a partir daquele conteúdo específico; (3) um exemplo de tom (explicitamente marcado como "adapta ao conteúdo real, não copies literalmente", para não sair robótico); (4) as regras factuais de sempre (material é fonte única, não inventar) mantidas no fim, antes do conteúdo. É texto simples em português, sem depender de formato de "system prompt" de nenhum fornecedor específico — funciona colado em qualquer IA (ChatGPT, Claude, Gemini, etc.), que era o pedido.

**Validação:** `npx tsc --noEmit` limpo; `ast.parse` limpo em `db.py`; testei a f-string do prompt isoladamente em Python (fora do worker completo) para confirmar que title/layers/content interpolam certo e não há chaves por escapar. Nada disto depende de recompilar o Rust — só o `db.py` (dev-mode Python, recarrega sozinho) e o frontend (precisa do rebuild do Tauri para o `.exe`, mas sem tocar em Cargo/capabilities desta vez).

### Registro — 2026-08-14 (ii): docx com layout de verdade + exportação unificada num .zip com 3 ficheiros (v0.42.0)

**Contexto:** o Leonardo testou a v0.41.0 e apontou dois problemas concretos, com screenshots: (1) o `.docx` gerado tinha o conteúdo literal `["Supabase é conhecido pelo público.", "Dentro da VPS..."]` — o repr de uma lista Python despejado como texto cru, em vez de bullets — e o layout em geral "parece que foi cuspido de um serviço web"; (2) ao clicar "Exportar" NÃO apareceu nenhum diálogo para escolher onde guardar — o ficheiro foi parar sozinho na última pasta usada numa sessão anterior (a mesma pasta onde guardou um transcript), sem qualquer interação visível. Ele também redefiniu o que quer da exportação: não dois botões separados, mas UM "Exportar" que produz 3 ficheiros — um `.md` com o conteúdo exato do editor + tudo o que foi adicionado, um `.docx` consolidado e bem desenhado, e um prompt em `.md`/`.txt` pronto para colar noutra IA — "talvez num zip".

**O que mudou (`App.tsx`):**
- **Unificação:** as duas cartas "Documento (.docx)" e "Pacote para IA (.md)" viraram uma única carta "Exportar (.zip)" com um botão só (`exportBundle()`), que busca os dois endpoints já existentes (`notebook_export` e `notebook_context_package_create`) em paralelo e empacota TRÊS ficheiros num `.zip` (biblioteca `jszip`, nova dependência): `{título}.md` (conteúdo cru exato, sem qualquer alteração — é a cópia fiel do editor + camadas escolhidas), `{título}.docx` (versão formatada), `{título} - prompt para IA.md` (o prompt, inalterado). Só se abre UM diálogo "guardar como", para o `.zip`.
- **Bug do repr de lista Python virar texto cru no docx:** nova função `tryExpandBracketList()` — deteta uma linha que é literalmente `["...", "..."]` (JSON válido, lista de strings) e expande cada item em bullet real, em vez de mostrar os colchetes e aspas cruas. Validado num teste isolado em Node: a mesma nota do screenshot do Leonardo agora produz bullets limpos (`"Supabase é conhecido pelo público."`, etc.) em vez do texto `["Supabase..." ...]`. **Nota:** a causa raiz — uma nota cujo corpo guardou o repr de uma lista Python em vez de texto formatado — continua por investigar (provavelmente um campo de lista de alguma origem de "Salvar no Caderno" que nunca passou por `_notebook_body_to_text`); esta correção só torna a EXPORTAÇÃO resiliente a esse dado, não corrige a origem do dado em si.
- **Layout do `.docx` deixou de usar só os estilos "Heading 1/2/3" default do Word e ganhou um desenho próprio:** título grande a azul-marinho (#1F3864) com subtítulo cinza-itálico "Exportado do UpexNote — {data}" por baixo, uma régua fina a separar do corpo, secções `##` com uma linha fina de separação e cor consistente, corpo do texto justificado com espaçamento de linha 1.25, margens de página de 1"/0.9", e rodapé com "UpexNote — Página X de Y" centrado (campos `PageNumber.CURRENT`/`TOTAL_PAGES` da biblioteca `docx`).
- **Diálogo "guardar como" não aparecer:** não consegui reproduzir nem confirmar a causa exata sem um Windows real à mão — a hipótese mais provável é a janela sem decorações (`"decorations": false` no `tauri.conf.json`) a interferir com o foco do diálogo nativo do Windows. Mitigação aplicada (não confirmada): `getCurrentWindow().setFocus()` chamado logo antes do `save()`, para garantir que a janela principal (e por extensão o diálogo que dela depende) está em primeiro plano. Se isto não resolver, o próximo passo é o Leonardo confirmar se o diálogo aparece ATRÁS da janela principal (Alt+Tab) em vez de não aparecer de todo — são bugs diferentes com correções diferentes.

**Nova dependência:** `jszip` (^3.10.1) em `package.json` — mais um motivo para rodar `npm install` antes do próximo build, além do `docx` já adicionado na v0.41.0.

**Validação:** `npx tsc --noEmit` limpo. Testei a geração real do `.docx` e do `.zip` num script Node isolado dentro do próprio `node_modules` do projeto (fora do runtime do Tauri, mas com as mesmas bibliotecas) — confirmei bytes gerados sem exceções e inspecionei o XML interno do `.docx` (`word/document.xml`) para confirmar que os bullets da lista-repr saíram como itens de lista reais (`<w:numPr>`), não texto cru. Não há como testar o fluxo completo (diálogo nativo do Windows, gravação em disco via `write_binary_file`) sem o build real do Leonardo.

### Registro — 2026-08-14: Exportar/Pacote para IA passam a gravar ficheiros reais (.docx / .md) (v0.41.0)

**Contexto:** o Leonardo apontou, com screenshots do painel de Exportação, que "Exportar" deveria gerar um .docx bonito e formatado, não só despejar o texto cru numa caixinha — e que "Exportar" e "Gerar pacote para IA" pareciam fazer a mesma coisa duplicada (ambos só copiavam texto para a área de transferência e mostravam num `<textarea>` readonly). Também apontou que o botão "Reopen" na lista de "pacotes gerados anteriormente" não fazia sentido nenhum — a funcionalidade devia ser um botão de exportar com escolha de onde guardar.

**O que mudou:** os dois fluxos deixam de mostrar prévia em caixinha/clipboard e passam a abrir o diálogo nativo `save()` do Tauri (`@tauri-apps/plugin-dialog`, já usado no projeto para `open()`) perguntando ONDE guardar, com nome sugerido a partir do título da nota:
- **"Exportar"** agora monta um `.docx` de verdade a partir do markdown que `notebook_note_export` já produzia (`db.py` — só usa `# `/`## `/`### ` para títulos e `- ` para listas, ver `_notebook_body_to_text`/`notebook_note_export`) — uma nova função `markdownToDocxParagraphs()` no frontend converte essas linhas em parágrafos/títulos/listas reais da biblioteca `docx` (heading styles, espaçamento, bullets), empacotados com `Packer.toBlob()` e gravados em disco pelo novo comando Rust `write_binary_file`.
- **"Gerar e guardar" (pacote para IA)** continua a gerar o prompt via `notebook_context_package_create` (inalterado no backend), mas agora grava-o em `.md` (ou `.txt`, ambos oferecidos como filtro no diálogo) através do novo comando Rust `write_text_file`, em vez de só copiar para a área de transferência.
- A lista "Pacotes gerados anteriormente" com o botão "Reopen" foi removida da UI — confuso e sem utilidade real segundo o feedback. O backend (`notebook_context_packages`/`notebook_context_package_item`) continua a existir e a gravar histórico no Postgres, só deixou de ter uma UI dedicada no frontend.

**Por que o .docx nasce no frontend e não no worker Python:** o worker não tem `python-docx` instalado, e mexer no `requirements`/spec do PyInstaller e validar que o build empacota a nova dependência não dava para confirmar nesta sessão de trabalho (sem toolchain Windows). Gerar o `.docx` no frontend com a biblioteca `docx` (npm) mantém a mudança inteiramente do lado que consigo validar com `tsc`, sem tocar no worker/PyInstaller.

**Mudanças de infraestrutura necessárias:**
- `package.json`: nova dependência `docx` (^9.7.1).
- `src-tauri/Cargo.toml`: sem novas dependências — os comandos `write_text_file`/`write_binary_file` usam só `std::fs::write`, de propósito, para não introduzir um plugin novo (`tauri-plugin-fs`) sem conseguir validar que compila no Windows do Leonardo.
- `src-tauri/capabilities/default.json`: já tinha `dialog:allow-open` (usado noutro sítio) — adicionada `dialog:allow-save` para o diálogo "Guardar como".
- `src-tauri/src/lib.rs`: dois comandos novos (`write_text_file`, `write_binary_file`) registados no `invoke_handler!`.

**Validação:** `npx tsc --noEmit` limpo. `npm install docx` correu sem erros nesta sessão (registo acessível). **Importante para o rebuild:** como há uma dependência nova no `package.json`, o Leonardo precisa correr `npm install` (não só `npm run build`/`rebuild-dev.ps1`) antes do próximo build, senão o `docx` não vai estar em `node_modules`. Sem toolchain Windows aqui para confirmar que o Rust compila com os dois comandos novos e a capability nova — mais uma vez, só um build real no Windows dele confirma.

### Registro — 2026-08-13 (x): balão flutuante ao selecionar texto — dicionário e comentário direto na palavra (v0.40.0)

**Contexto:** depois da v0.39.0 (tabs reformuladas + Glossário com API), o Leonardo disse que a solução do Glossário "não era bem isso que queria" e explicou o fluxo real, com screenshot mostrando o botão "Look up" ainda meio cortado no formulário manual do sidebar: selecionar (ou clicar com o botão direito) numa palavra deveria abrir um balãozinho ali mesmo, oferecendo "dicionário" e "comentário"; o dicionário abre outra caixinha com a definição, que ao salvar aparece como entrada no painel de Glossário à direita; o comentário é a mesma ideia mas para anotação (que ele descreveu como "já vem" — ou seja, já existente no fluxo antigo por seleção, só faltava o balão visual).

**O que mudou (`App.tsx`, `App.css`, `i18n.ts`):** `handleEditorSelect` agora também guarda a posição da seleção (`selectionRect`, via `range.getBoundingClientRect()`) e o editor ganhou `onContextMenu` (clique direito com seleção ativa abre o balão em vez do menu nativo do SO). Um novo componente flutuante (`position: fixed`, `.nb-selection-popup`) aparece acima da seleção com 2 botões — "Dicionário" (`BookOpen`) e "Comentário" (`MessageSquarePlus`). Clicar em "Dicionário" abre um cartão com a palavra selecionada, um botão de busca que consulta `dictionaryapi.dev` (reaproveitando a mesma função de busca da v0.39.0, agora extraída para `fetchDictionaryDefinitions()` partilhada), e ao encontrar definição mostra campos editáveis + "Adicionar ao glossário" — que grava via `notebook_glossary_create` com `source: "dictionaryapi.dev"` e recarrega a lista do painel lateral. Clicar em "Comentário" abre o MESMO fluxo de anotação que já existia (`selectionInfo`/`annotationDraft`/`submitAnnotation`), só que agora como balão flutuante posicionado na seleção em vez de só aparecer dentro da aba lateral — ao salvar, continua a aparecer também na aba Anotações/Referências do painel direito, como antes. O formulário manual de adicionar termo (que já existia na aba Glossário do sidebar) foi mantido como alternativa para quando não há texto selecionado, mas o botão de busca dele passou a ser só ícone (sem rótulo de texto) — resolve o corte reportado ("Look up" cortado na borda do painel estreito).

**Validação:** `npx tsc --noEmit` limpo. Sem ambiente Windows aqui para confirmar visualmente o posicionamento do balão (usa `getBoundingClientRect()` do range de seleção — deve funcionar em qualquer resolução de janela, mas fica por confirmar que não sai da área visível em seleções perto da borda da janela — não implementei clamping de viewport nesta primeira passagem).

### Registro — 2026-08-13 (ix): tabs do painel lateral reformuladas + Glossário passa a consumir dicionário externo (v0.39.0)

**Contexto:** o Leonardo confirmou que a Fase A + a correção de imports lentos melhoraram mesmo o tempo de carregamento, mas apontou duas coisas que já tinha dito antes e que eu tinha só registado como "pendente" em vez de resolver: "essa parte de referencia esta meio estranha ainda nao esta quebrado os icones nao esta legal" (com screenshot mostrando a aba "Exportar" cortada para "Ex") e "isso te falei sobre dicionario d eapi tbm nao me ouviu". Desta vez implementado no mesmo turno, sem voltar a registar como pendente.

**Tabs do painel lateral (`App.tsx` + `App.css`):** as 6 abas (Anotações/Referências/Ligações/Glossário/Exportar/Versões) tinham ícone + rótulo de texto + contagem espremidos em `flex:1` dentro de um painel de 260px — com 6 rótulos isso não cabe, daí o "Ex" cortado. Trocado para abas só com ícone (maior, 15px) + um badge numérico sobreposto (só aparece quando a contagem é >0) + `title` com o rótulo completo em tooltip. CSS: aba ativa agora usa `box-shadow: inset 0 -2px 0 var(--accent)` em vez de mudar só a cor de fundo, e há um `:hover` novo — mais claro visualmente qual aba está selecionada, que era outra parte do "não está legal".

**Glossário com dicionário externo (`App.tsx`, `i18n.ts`):** o utilizador digita o termo e clica "Buscar no dicionário" (ou Enter no campo termo) — chama `dictionaryapi.dev` diretamente do frontend (`fetch`, sem passar pelo worker Python: `tauri.conf.json` já tem `csp: null`, é uma API pública sem chave/segredo, e ao contrário das chamadas LLM do `formatting.py` não há nada a proteger aqui), pega até 3 definições (com classe gramatical quando disponível) e preenche automaticamente o campo definição + idioma. Idioma por omissão `pt-BR` (a API suporta; código não normalizado como "pt" ou vazio cai para `pt-BR`, outro valor é usado literalmente — permite `en`, `es`, etc.). Continua a existir a entrada manual como estava — só passa a ser opcional, não a única via. `source` (campo que já existia no schema, sempre "manual" até agora — ver comentário antigo em `db.py`/`App.tsx` linha ~1859) passa a gravar `"dictionaryapi.dev"` quando a definição vem da consulta; qualquer edição manual do termo/definição/idioma depois da consulta limpa o `source` de volta para `null` (evita etiquetar como "API" um texto que o utilizador reescreveu). Entradas com fonte não-manual mostram um ícone de livro pequeno ao lado do termo na lista, com tooltip indicando a fonte. Nenhuma mudança em Rust/Python foi necessária — `notebook_glossary_create` já aceitava `source` de ponta a ponta (Rust → CLI → `db.py`), só nunca tinha sido usado com um valor real.

**Validação:** `npx tsc --noEmit` limpo (0 erros) na cópia local após as mudanças. Sem ambiente Windows/Tauri nesta sessão para testar o `fetch` real contra `dictionaryapi.dev` a partir do webview — a app deve ser testada pelo Leonardo para confirmar que o pedido de rede sai do WebView2 sem bloqueio (não deveria haver, dado `csp: null`, mas fica por confirmar num build real).

**Lição repetida (mesma da Fase A):** isto é a segunda vez que um pedido explícito do Leonardo tinha ficado só registado como "pendente" em vez de implementado. Registada de novo na memória do projeto para não repetir uma terceira vez.

### Registro — 2026-08-13 (viii): Fase A implementada — worker persistente (v0.38.0)

**Contexto:** depois das Fases B+C e de dois bugs de infraestrutura de build corrigidos, o Leonardo foi direto: "nao tem sentido a fase A vamos resolver isso logo ficar remendando nao resolve... confiei e parei de cobrar mas vc nao esta obdecendo." Ele tinha razão — eu tinha classificado a Fase A como "projeto à parte" quando já era claramente o que estava a causar a lentidão residual. Implementada agora, sem mais adiamento.

**O que mudou, lado Python (`services/worker/transcription/cli.py`):** novo comando `serve` — um processo que fica vivo a ler pedidos de uma linha JSON no stdin (`{"id", "argv", "stdin"}`) e a responder por linhas JSON no stdout (`{"id", "line"}` zero ou mais vezes, seguido de `{"id", "done": true}`). Os handlers `cmd_*` não mudam NADA — continuam a escrever em `sys.stdout` e a ler de `sys.stdin` exatamente como sempre — o loop troca essas duas streams por `io.StringIO` à volta de cada pedido (capturando o que foi escrito, alimentando o que seria lido) e restaura as reais no `finally`. Isto é o que permite, por exemplo, `notebook-note-update` continuar a ler o corpo da nota por "stdin" mesmo com o stdin REAL do processo ocupado a servir de canal de pedidos do loop. Um erro num pedido (comando desconhecido, exceção no handler) fica isolado a esse pedido — não derruba o loop. `_build_handlers()` foi extraído de `main()` para ser reutilizado por `cmd_serve()` sem duplicar a tabela de ~90 comandos.

**O que mudou, lado Rust (`apps/desktop/src-tauri/src/lib.rs`):** `run_cli`, `run_cli_async` e `run_cli_stdin_async` — os três pontos centrais por onde passam ~60 dos comandos Tauri (todo o Caderno, Biblioteca, document_item/delete, etc.) — deixaram de fazer `Command::spawn()` a cada chamada. Agora chamam `call_worker()`, que mantém UM processo `serve` vivo num `static WORKER: OnceLock<Mutex<Option<PersistentWorker>>>` (mesmo padrão já usado para o `TUNNEL_KEEPER` — o `Child` fica preso ao static; quando a app fecha, mesmo em crash, o pipe do stdin fecha e o `serve` sai sozinho por EOF, sem processos órfãos), reenvia o pedido, lê linhas até `done`, e devolve a mesma forma de resposta que sempre devolveu — nenhum dos ~60 comandos Tauri precisou de ser tocado individualmente. Se a comunicação falhar (pipe partido, worker morreu a meio), o worker é morto e a PRÓXIMA chamada arranca um novo do zero.

**Ficou de fora, propositadamente:** `transcribe` (streaming de progresso via eventos Tauri), `oauth`, `db-check`, `api-reset-*`, `support-*`, `account-*`/`admin-*` com prova de senha — continuam a spawnar o próprio processo. Streaming precisa de um processo dedicado por natureza; os outros são ações raras onde isolamento por chamada é uma escolha de segurança deliberada (segredo não fica na memória de um processo de longa duração), não uma questão de performance.

**Validação (fora do ambiente do Leonardo, sem toolchain Rust para Windows disponível nesta sessão):** o lado Python foi testado de ponta a ponta — arranquei `python -m transcription.cli serve` como subprocesso real, enviei 3 pedidos sequenciais pela MESMA ligação stdin/stdout (incluindo um comando inválido de propósito, para confirmar que o loop sobrevive a um erro e continua a responder aos pedidos seguintes) e um pedido com corpo por "stdin" (confirmando que o handler recebeu o valor certo através do envelope, não do stdin real do processo). Os três comportamentos-chave confirmados: processo único reaproveitado entre pedidos, isolamento de erro por pedido, e o truque de troca de stdin/stdout a funcionar. `python3 -c "import ast; ast.parse(...)"` limpo em `cli.py`. Lado Rust: `tsc`/`ast` não se aplicam — validado por revisão manual cuidadosa (contagem de chaves balanceada, mesmo padrão de lifetime/borrow já usado no `TUNNEL_KEEPER` do próprio ficheiro) — só um build real no Windows do Leonardo confirma definitivamente que compila.

**Isto é a correção estrutural, não mais um remendo:** elimina o custo de arranque de processo (interpretador + imports) em praticamente toda ação do Caderno e da Biblioteca — não só a lentidão reportada, mas a causa que a análise arquitetural de 2026-08-13 identificou como raiz de primeira ordem.

### Registro — 2026-08-13 (vii): "voltou a demorar" — imports pesados de motores de transcrição a correr em TODO comando do worker

**Sintoma:** depois de corrigir o worker sidecar fantasma (registo anterior), o Leonardo confirmou melhoria nas outras partes mas reportou que a app "voltou a demorar" a abrir notas — pior do que com o worker empacotado (mesmo esse estando desatualizado).

**Diagnóstico:** `registry.py` importava `assemblyai`, `whisper_openai`, `deepgram` e `gpt4o_openai` incondicionalmente no topo do ficheiro. `cli.py` importa `registry` no arranque — ou seja, TODO comando do worker (mesmo `notebook-note-open`, que nada tem a ver com transcrição) pagava esses imports. `whisper_openai.py` e `gpt4o_openai.py` importam o SDK da OpenAI (pesado); `whisper_openai.py` e `deepgram.py` importam `audio_chunks.py`, que importa `numpy` e `av` (PyAV — dezenas de DLLs de ffmpeg). Isto sempre existiu, mas só ficou visível agora que a app corre sempre em modo desenvolvimento (`python -m transcription.cli`, sem o worker empacotado/congelado) — cada spawn de processo é um interpretador Python a arrancar do zero e a pagar estes imports pesados, mesmo para abrir uma nota.

**Correção:** `registry.py` agora usa wrappers (`_run_assemblyai`, `_run_whisper_openai`, `_run_deepgram`, `_run_gpt4o_openai`) que só importam o módulo do motor especifico quando `run(...)` é mesmo chamado (comando `transcribe`) — os restantes comandos deixam de pagar numpy/av/openai. `formatting.py` (motores de formatação) já era leve (só `requests`) — manteve-se eager.

**Nota honesta:** isto reduz uma fatia real do custo, mas não elimina o arranque do processo/interpretador em si — essa é exatamente a Fase A da análise arquitetural (worker persistente), ainda não feita. A primeira abertura de uma nota nesta sessão (sem cache local ainda) continua a pagar rede + spawn; só deixa de pagar os imports desnecessários por cima disso.

**Feedback pendente do Leonardo, ainda não implementado (ficou registado, não é regressão):** o "Glossary" da nota é hoje uma lista manual (termo/definição digitados à mão) — o Leonardo apontou corretamente que isto não é uma anotação, é um dicionário de verdade, e portanto devia consumir uma API de dicionário externa (sugestão dele, API a escolher) em vez de só guardar o que a pessoa escreve. Também pediu: collapse na árvore de notebooks/notas, e repensar References/Links/Glossary/Export como um painel que só abre ao clicar (em vez de ocupar sempre a lateral). Fica registado para próxima ronda — não implementado agora para não desviar do foco da correção de performance/arquitetura em curso.

### Registro — 2026-08-13 (vi): "notebook-note-open: invalid choice" — worker sidecar fantasma, causa raiz real no rebuild-dev.ps1

**Sintoma:** depois de sincronizar as fases B+C (registo anterior), o Leonardo testou e apanhou `notebook-note-open` como "invalid choice" na lista de comandos do CLI — a nota abria vazia, e o apagar de notas pareceu lento/travado (efeito em cascata do mesmo erro, não um bug novo).

**Diagnóstico (verificado por inspeção direta do disco, não suposição):** `apps\desktop\src-tauri\target\release\worker\upexnote-worker.exe` tinha data de **9 de agosto**, três dias mais velho que o `upexnote.exe` recém-compilado — ou seja, o app continuava a preferir um worker empacotado (PyInstaller) congelado numa versão de `cli.py` anterior a todas as correções desta sessão, em vez do Python de desenvolvimento com o código atual (`worker_command()` em `lib.rs` prefere sempre o sidecar empacotado, se existir). A causa NÃO foi falta de cuidado ao aplicar as fases B/C — foi um bug no `rebuild-dev.ps1` que eu próprio escrevi: ele só apagava a cópia de SAÍDA (`target\release\worker`), mas o Tauri copia os `resources` do `tauri.conf.json` (`"worker": "worker"`) para `target\release\` em TODO build — mesmo com `--no-bundle` — a partir da cópia FONTE em `apps\desktop\src-tauri\worker\`. Essa fonte ficou parada desde a última vez que alguém correu `build_worker.ps1`/`make_portable.ps1` (9 de agosto, para tentar um zip portátil) e nunca foi tocada de novo — por isso apagar só a saída nunca resolvia: o build seguinte "ressuscitava-a" a partir da fonte velha, sem aviso nenhum.

**Correção:** `rebuild-dev.ps1` agora apaga as DUAS cópias (`target\release\worker` e `src-tauri\worker`) sempre, antes de recompilar — garante que o app corre sempre em modo desenvolvimento (`python -m transcription.cli`, código fresco do repo) a menos que alguém explicitamente corra `make_portable.ps1` de novo (esse continua a reconstruir as duas cópias a partir do zero via PyInstaller, para quando for preciso mesmo o zip portátil/instalador). Também limpei manualmente a cópia fonte (`src-tauri\worker`) que estava parada — movida para `_to_delete\worker-source-stale-2026-08-13\` (não apago diretamente por não ter permissão de terminal na máquina do Leonardo, só o bridge de ficheiros).

**Impacto:** sem isto, qualquer comando novo do CLI (não só `notebook-note-open`) ia continuar a falhar silenciosamente com "invalid choice" sempre que o worker empacotado estivesse mais velho que o `cli.py` do repo — um risco estrutural para qualquer correção futura, não só esta.

**Correção (ii), no mesmo dia — apagar a fonte por inteiro partiu o build:** a primeira correção acima apagava `src-tauri\worker` por inteiro. O build seguinte falhou logo: `resource path 'worker' doesn't exist` — o Tauri valida em COMPILE-TIME (build.rs) que esse caminho existe, porque está declarado em `tauri.conf.json` como `bundle.resources`, mesmo com `--no-bundle`. Corrigido: `src-tauri\worker` passa a ser sempre mantida (o Tauri só exige que o CAMINHO exista), só o CONTEÚDO é apagado — uma pasta vazia satisfaz o build, e sem `upexnote-worker.exe` lá dentro o runtime continua a cair no fallback de desenvolvimento. `target\release\worker` (a cópia de saída) continua a poder ser apagada por inteiro sem problema, porque essa não é validada em compile-time.

### Registro — 2026-08-13 (v): correção arquitetural do Caderno, fases B+C (v0.37.0)

**Contexto:** depois de uma sequência de correções pontuais (v0.36.2 a v0.36.6), cada uma resolvendo um sintoma novo que o Leonardo encontrava no teste seguinte, ele pediu explicitamente para parar de remendar e analisar a fundo — "não posso comercializar" no estado atual. A análise (`docs/ANALISE_ARQUITETURA_CADERNO_2026-08-13.md`) identificou duas causas raiz sistémicas, não um bug: (1) cada ação do utilizador dispara um processo do SO inteiro do zero (abrir uma nota = 6 processos/handshakes separados); (2) `NotebooksView` e `LibraryView` ficam ambos montados o tempo todo, cada um com a sua própria cópia de coleções/notas, sem fonte única de verdade. Depois de apresentar a análise, o Leonardo deu liberdade para administrar a correção: "gerencia com responsabilidade, vc é um grande especialista, te dou a liberdade de administrar essa correção."

**Fase B — endpoint composto (elimina 6 processos → 1 ao abrir nota):** nova função `db.notebook_note_open(note_id, user_id)` que faz UMA ligação e devolve item + anotações + referências + links + keywords + glossário juntos (antes: `notebook_note_item`, `notebook_annotation_list`, `notebook_reference_list`, `notebook_links`, `notebook_keyword_list`, `notebook_glossary_list` — 6 chamadas, 6 processos, 6 handshakes SSH/Postgres). Novo comando CLI `notebook-note-open` (`cli.py`) e novo comando Tauri `notebook_note_open` (`lib.rs`), registado no `invoke_handler`. `openNote()` (`App.tsx`) passou a fazer 1 `invoke` em vez de 6.

**Fase C — store partilhado (elimina a duplicação de estado entre vistas):** novo módulo de estado fora do React (`nbTreeState` + `nbTreeSubscribe`/`nbTreeSet`/`nbTreeUpdate`, junto dos helpers de cache do Caderno) — a única fonte de verdade para `collections`/`notes`. `NotebooksView` e `LibraryView` leem-no via `useSyncExternalStore`; qualquer mutação numa vista (criar coleção no diálogo "Salvar no Caderno", apagar nota na árvore, etc.) fica visível na outra vista no mesmo instante, sem F5 manual e sem o canal ad-hoc que existia só para o caminho "abrir no caderno". Isto resolve na origem o bug "criei o TESTE e não apareceu no Notebook" — já não é possível as duas vistas divergirem, porque não há duas cópias.

**Cuidado técnico registado:** a primeira versão do refactor da Fase C tinha uma mutação encadeada (`setCollections` chamando `setNotes` dentro do seu updater, em `deleteCollection`) que seria reentrante sobre o estado do módulo — a publicação de fora sobrepunha a de dentro e perdia a exclusão das notas. Corrigido antes de sincronizar: `deleteCollection` agora faz uma única chamada a `nbTreeUpdate` que muda `collections` e `notes` atomicamente.

**Fases A e D (worker persistente; cache unificada) ficam para depois** — A é a mudança de maior alavancagem (elimina o custo de processo em todo o app, não só no Caderno) mas é um projeto à parte, não uma correção de sessão; D consolida a cache numa camada coerente depois de B/C estabilizarem.

**Validação:** `tsc --noEmit` limpo; `python3 -c "import ast; ast.parse(...)"` limpo em `db.py` e `cli.py`; contagem de chaves/parênteses em `lib.rs` conferida manualmente (sem `cargo` para Windows disponível neste ambiente de análise).

**Pendente, não esquecido:** `Cargo.toml` (`[package] version`) continua desalinhado da versão da app — ficheiro não está espelhado nesta sessão; corrigir na próxima vez que for sincronizado.

### Registro — 2026-08-13 (iv): "Loading..." logo a seguir a guardar no Caderno — a nota nunca tinha cache (v0.36.6)

**Contexto:** depois da correção anterior (navegar para o Caderno ao gravar), o Leonardo reparou que a nota recém-gravada ficava presa em "Loading..." — questionou porque é que teria de "carregar de novo" um conteúdo que "já tinha carregado antes" (na prévia da Biblioteca), e pediu para rever a arquitetura em vez de ir só remendando.

**Diagnóstico:** tinha razão, mas a causa era mais específica do que "a app é lenta em geral" — o cache local (stale-while-revalidate) só acelera a **segunda** vez que uma nota é aberta; para a nota que a pessoa acabou de gravar, é literalmente a primeira vez que ela existe como nota do Caderno, por isso nunca podia ter cache. `notebook_save_document` (CLI) devolvia só `{id, existed}` — sem o conteúdo — obrigando `openNote()` a fazer uma segunda viagem completa pelo túnel SSH só para mostrar o que o próprio cliente **já tinha na mão** (o documento estruturado que gerou a prévia).

**Correção (elimina a viagem extra, não só disfarça):** `cmd_notebook_save_document` (`services/worker/transcription/cli.py`) passou a chamar `db.notebook_note_item()` internamente e devolver a nota completa no mesmo `{"type": "ok", ...}` — sem pedido extra ao SGBD além do que já faria de qualquer forma para confirmar a gravação. `confirmSaveToNotebook()` (`App.tsx`) grava essa nota já recebida na cache local (`writeNbNoteCache`) **antes** de navegar para o Caderno — quando `openNote()` corre a seguir, encontra logo a cache e pinta na hora; o pedido fresco ainda corre por trás, só para confirmar, sem bloquear nada.

**Sobre "viver de remendo":** distinção que vale registar — uma nota aberta pela **segunda** vez (ou mais) já era instantânea desde o registo de 2026-08-11 (ii), porque a cache local já existia nessa altura. O que faltava era só este caso específico (primeira vez que uma nota existe, vinda de uma gravação da Biblioteca) — e este registo fecha exatamente essa lacuna, na origem (o backend deixa de forçar dois pedidos onde um chega), não com um efeito de carregamento a disfarçar a espera.

**Validação:** `python3 -c "import ast; ast.parse(...)"` limpo no `cli.py`; `tsc --noEmit` limpo.

**Impacto em dados, custo ou privacidade:** nenhum — a mesma nota, só devolvida uma vez em vez de duas.

### Registro — 2026-08-13 (iii): "Salvar no Caderno" não navegava para o Caderno, coleção nova não aparecia (v0.36.5)

**Contexto:** o Leonardo criou um caderno novo ("TESTE") a partir do diálogo "Salvar no Caderno" da Biblioteca, gravou lá um documento e — em vez de ir parar ao Caderno com a nota já aberta — ficou na prévia da Biblioteca, sem qualquer sinal de que a gravação (ou a ligação ao caderno novo) tinha acontecido. Ao mudar manualmente para a aba Notebooks a seguir, o "TESTE" aparecia, mas vazio.

**Causa 1 — `confirmSaveToNotebook()` nunca navegava.** Gravava a nota, atualizava o estado local da Biblioteca (`notebook_note_id` no documento), fechava o diálogo — e parava aí. Já existia `onOpenInNotebook` como prop do `LibraryView` (usado noutro botão, "Abrir no Caderno"), só não estava a ser chamado aqui. Corrigido: chama `onOpenInNotebook(obj.id)` no fim, tal como o resto do produto já faz noutros pontos de entrada.

**Causa 2 — a árvore do Caderno não sabia da coleção nova.** A `NotebooksView` só corre o `load()` completo uma vez por montagem (`loadedOnce`) — e como as vistas não desmontam ao trocar de aba, uma coleção criada minutos antes por OUTRA aba (o diálogo da Biblioteca) nunca chegava a essa árvore sem um F5 manual. Corrigido: o efeito que trata um pedido "abrir no caderno" vindo de fora (`openRequest`) agora também dispara um `load()` real (não em cache) em paralelo com o abrir da nota — é o único ponto em que vale a pena pagar esse pedido extra, porque é exatamente o momento em que a árvore pode estar desatualizada por ação de outra aba.

**Pedido à parte — sensação de "travado" ao criar coleção/gravar.** Os botões já mostravam texto "A guardar..." durante a espera, mas sem nenhum indicador visual de progresso parecia impossível distinguir de travado. Adicionado o mesmo spinner giratório já usado noutros pontos do produto (`<span className="spinner" />`) aos botões "Criar" e "Guardar" desse diálogo.

**Validação:** `tsc --noEmit` limpo.

**Nota sobre a lentidão em si:** o `load()` extra que corrigiu a causa 2 é, ele próprio, uma viagem completa pelo túnel SSH — inevitável neste caso específico (é a única forma de a árvore saber de uma mudança feita por outra aba), mas continua a ser latência de rede real, não frontend. Reduzir isto de forma mais estrutural (p.ex. invalidar só a coleção que mudou, ou um canal de eventos entre abas em vez de recarregar tudo) fica para uma iteração futura, se a latência do túnel continuar a incomodar mesmo depois destas correções.

### Registro — 2026-08-13 (ii): CRUD do Caderno lento e com estado inconsistente após apagar/criar (v0.36.4)

**Contexto:** primeiro teste real da v0.36.3 (build via `scripts/rebuild-dev.ps1`, ver registo do script mais abaixo). O Leonardo reportou, com prints: apagar uma nota demorou muito e, mesmo depois de "apagado", a nota continuou a aparecer com um erro (`Nota #2 não encontrada`) por cima de outra nota já aberta; criar uma nota nova deu erro com o cursor a mostrar o ícone de bloqueio (🚫). Considerou o produto "não aceitável" neste estado — merece prioridade.

**Causa raiz, duas coisas diferentes a somar:**

1. **`error` (estado único, partilhado por toda a `NotebooksView`) nunca era limpo ao abrir uma nota nova.** `openNote()` não tinha `setError("")` no início (ao contrário de `load()`, `createNote()`, etc.) — por isso um erro de uma tentativa anterior (nota já apagada, `Nota #2 não encontrada`) ficava pendurado no ecrã por cima do conteúdo de uma nota completamente diferente, aberta com sucesso a seguir. Corrigido: `openNote()` agora limpa `error` logo no início.

2. **A cache local de uma nota apagada não era removida.** `readNbNoteCache(id)` (stale-while-revalidate, adicionado ontem) continuava a devolver o corpo antigo de uma nota já apagada no servidor, porque `deleteNoteRow()` nunca chamava `localStorage.removeItem` para essa entrada — se esse id fosse reaberto por qualquer via, a nota "ressuscitava" visualmente antes do pedido fresco confirmar o 404. Corrigido com `removeNbNoteCache(id)`, chamado em `deleteNoteRow()` e em `deleteCollection()` (que arrasta as notas da coleção apagada).

**A lentidão em si — causa raiz separada, não é a latência do túnel SSH por si só.** `createNote`, `deleteNoteRow`, `deleteCollection`, `createCollection` e `saveNote` chamavam todos `await load()` — um `notebook_tree` completo — **depois** de cada mutação, e só prosseguiam (abrir a nota nova, mostrar a lista sem a apagada, etc.) depois desse pedido extra terminar. Cada clique pagava sempre dois pedidos em série pelo túnel, um atrás do outro, quando só um era necessário. Corrigido para todos os cinco: a mutação em si atualiza o estado em memória (`notes`/`collections`) e a cache local imediatamente a partir da resposta que o próprio `invoke` já trouxe — sem esperar por mais nada — e um `load(true)` corre a seguir **em fundo, sem bloquear** (`void load(true)`, não `await`), só para reconciliar em silêncio qualquer divergência (outra sessão a editar a mesma conta, posição exata, etc.). O ícone de bloqueio ao criar era o cursor por omissão de um botão `disabled` (o botão "+" fica desabilitado enquanto `busyCollectionId` está definido) — com o pedido a demorar por causa do duplo round-trip, parecia estar preso; deve deixar de se notar com o caminho mais curto.

**Validação:** `tsc --noEmit` limpo.

**Impacto em dados, custo ou privacidade:** nenhum — só passa a confiar na resposta que o próprio pedido de escrita já devolve, em vez de a redescobrir com uma leitura extra a seguir. Sem risco de perda de dados: a reconciliação em fundo continua a correr, é só não bloqueante.

### Registro — 2026-08-13: script `rebuild-dev.ps1` — dois bugs de encoding no próprio script (não no produto)

Ao tentar dar ao Leonardo um único ficheiro para substituir os blocos de comandos PowerShell fragmentados (pedido dele — "não me mande caminhos e comandos picados"), o script `scripts/rebuild-dev.ps1` em si apanhou os mesmos dois problemas que já se tinham visto no produto:

1. **Sem BOM UTF-8** — o Windows PowerShell 5.1 (não o PowerShell 7) lê `.ps1` sem BOM na code page ANSI do sistema, não UTF-8; os acentos do script ("é", "não", "há") viraram sequências de dois caracteres (mojibake), o que partiu uma string a meio e derrubou o parser inteiro (`Missing closing '}'`, `The string is missing the terminator`). Corrigido gravando o ficheiro como UTF-8 com BOM e, por segurança extra, removendo todos os acentos do texto do próprio script.
2. **`$ErrorActionPreference = "Stop"` global + `taskkill` sobre um processo já fechado.** Quando o processo já não existe, `taskkill` escreve no stderr; com `ErrorActionPreference = Stop`, isso vira erro fatal do PowerShell mesmo com `2>$null` a tentar suprimir. Corrigido a correr o `taskkill` dentro de `cmd /c "... >nul 2>&1"`, para o aviso nunca chegar a ser visto pelo PowerShell.

**Validação:** o Leonardo correu o script depois da correção e o build completou (`tauri build --no-bundle`) sem erros.

### Registro — 2026-08-13: colunas redimensionáveis no Caderno + editar nota direto na árvore (v0.36.3)

Retomados os 4 pontos deixados pendentes na sessão anterior (imagem anotada do Leonardo, ver histórico abaixo — **não tinha nada a ver com idioma**, isso foi um mal-entendido já corrigido):

1. **Editar+apagar na nota da árvore.** Cada linha de nota (`apps/desktop/src/App.tsx`, `renderCollection()`, classe `nb-note-row-wrap`) ganhou um ícone de lápis (`Pencil`, `nbEditNote`) ao lado do de lixo já existente. O lápis abre a nota (`openNote`) e foca já o campo do título (`titleInputRef`), pronto a editar sem cliques extra. Não foi implementado rename inline separado (exigiria carregar o corpo completo da nota só para reescrever o título) — abrir+focar cobre o pedido com muito menos risco.

2. **Auto-colapso do menu principal vindo da Biblioteca.** Confirmado por leitura de código: já estava coberto desde v0.36.2, porque o `useEffect` reage a `view === "notebooks"` independentemente de como se chegou lá (clique direto ou `openInNotebook()` a partir da prévia da Biblioteca) — ambos os caminhos passam por `navTo("notebooks")` → `setView`. Nenhuma alteração necessária; falta só o Leonardo confirmar visualmente amanhã.

3 e 4. **Colunas redimensionáveis (árvore | editor | painel lateral), com pega de arrastar.** Implementado como um único mecanismo: `nbSidebarWidth` e `nbSidePanelWidth` (estado em píxeis, limites 200–480 e 220–460, guardados em `localStorage` como `upexnote-nb-sidebar-width`/`upexnote-nb-panel-width`), aplicados via `style` inline nos containers `.nb-sidebar` e `.nb-side-panel` (a inline style ganha às regras fixas do `App.css`, que ficam como default de primeira execução). Duas pegas novas (`.nb-resizer`, `startResize("sidebar"|"panel")`) — uma entre a árvore e o editor, outra entre o corpo da nota e o painel lateral (só aparece quando `nbPanelOpen`). Arrastar atualiza o estado ao vivo via `mousemove`/`mouseup` na `window`; a persistência corre num `useEffect` a reagir ao estado (não dentro do `onUp`, para não gravar um valor desatualizado por fecho de closure). O botão liga/desliga do painel lateral (`nbPanelOpen`/`PanelRight`) manteve-se — a largura ajustável é um mecanismo adicional, não substitui o esconder/mostrar.

**Validação:** `tsc --noEmit` limpo depois de cada lote.

**Impacto em dados, custo ou privacidade:** nenhum — só preferências de layout locais, sem dado novo tratado.

### Registro — 2026-08-12: worker.exe antigo (bundled) a mascarar comandos novos, encoding UTF-8 no stdin do worker, atalhos de edição no Caderno

**Contexto:** o Leonardo testou um build real (v0.36.0) e apanhou dois defeitos concretos, mais uma lista de pedidos de UX numa imagem anotada.

**1) Comandos novos "não existiam" (`invalid choice: 'notebook-note-version-create'` e outros).** Causa: `worker_command()` (`lib.rs`) prefere sempre `<pasta do exe>\worker\upexnote-worker.exe` (sidecar PyInstaller já empacotado) em vez do Python do repositório, quando esse ficheiro existe — e um `tauri build --no-bundle` **não** mexe nessa pasta, por isso um `worker.exe` de um build cheio anterior (9 ago) ficava para trás, ignorando todo o código novo. Não há correção de código para isto — é preciso apagar `target\release\worker` antes de recompilar, para o fallback de dev (`python -m transcription.cli`, que lê sempre o código atual) assumir. A pasta só conseguiu ser apagada depois de matar os processos (`taskkill /F /IM upexnote.exe`, `taskkill /F /IM upexnote-worker.exe`) — o `Remove-Item -Recurse -Force` sem `-ErrorAction Stop` **não** falha visivelmente quando há ficheiros bloqueados; imprime "apagada" mesmo sem apagar nada.

**2) Acentuação corrompida (mojibake) só nas notas do Caderno, não na prévia da Biblioteca.** A prévia sai só por `stdout` (stream NDJSON); o corpo da nota entra pelo `stdin` do processo do worker. No Windows, sem consola real associada, o Python assume por omissão a code page ANSI do SO em vez de UTF-8 — só corrompe o caminho de `stdin`, exatamente o padrão relatado. Corrigido com `PYTHONUTF8=1` e `PYTHONIOENCODING=utf-8` no `Command` do worker em `worker_command()` (aplica-se aos dois caminhos, sidecar ou dev). Notas já gravadas antes desta correção **ficam corrompidas na base** — o texto errado já foi persistido; só notas novas (ou reescritas) saem limpas.

**3) Pedidos de UX da imagem anotada (v0.36.0):** endereçados os itens mais diretos — (a) menu principal esquerdo recolhe sozinho ao entrar em Notebooks (só recolhe, nunca reabre sozinho); (b) cada nota na árvore ganhou um ícone de lixo visível ao passar o rato, para apagar sem ter de abrir a nota primeiro; (c) a caixa do corpo da nota ficou com fundo mais claro (`--panel` em vez de `--panel-2`), borda com mais contraste e sombra subtil, para legibilidade. **Ainda por fazer** (precisam de mais desenho, não são um ajuste rápido de CSS): painel do editor com divisão redimensionável/recolhível de forma dinâmica, e o Glossário ligado a uma API de dicionário externo (sinónimos/definições) — ficam para a próxima leva, combinado com o Leonardo.

**Nota:** o idioma inglês visto nalguns prints (v0.36.1) não é um bug de tradução — o `localStorage` do próprio perfil da app tinha `upexnote-lang=en` guardado de um teste anterior (o valor por omissão é `pt`); troca-se em Definições → idioma.

**Validação:** `tsc --noEmit` limpo depois dos três ajustes de UX.

**Impacto em dados, custo ou privacidade:** nenhum dado novo tratado; o ponto 2 é uma correção de integridade de texto já guardado por utilizadores (sem alterar quem tem acesso a quê).

### Registro — 2026-08-11 (ii): recuperação de prévia não gravada, criar caderno no diálogo de destino, cache de sessão no Caderno

**Contexto:** feedback de teste real do Leonardo num build antigo (v0.31.0 — antes de toda esta sessão; ver nota no fim). Três queixas concretas, endereçadas aqui:

**1) Prévia gerada "desaparecia" ao reabrir o item.** Investigação confirmou: o schema, `insert_document` e a releitura da Biblioteca (`library_item` → `detail.documents`) já estavam corretos — o defeito era que `insert_document` é *best-effort* (nunca levanta) e `cli.py` já respondia `saved: doc_id is not None`, mas o frontend nunca lia esse campo — sempre mostrava "prévia criada com sucesso" mesmo quando a gravação falhava, e o conteúdo já gerado (custo de API já pago) ficava só na resposta do evento, nunca chegava a ser persistido nem reaproveitado. Correção: novo comando `document-save` (CLI) / `document_save` (Tauri) que recebe o documento **já gerado** (guardado em memória no cliente como `pendingDocument`) e só faz o INSERT — nunca chama a IA de novo. Quando `saved: false` chega, a UI mostra um aviso persistente com botão "Tentar guardar de novo"; ele fica visível na secção de prévias enquanto não gravar, mesmo se a pessoa navegar para outro item e voltar.

**2) Diálogo "Salvar no Caderno" só oferecia o caderno padrão.** Adicionado um "Criar novo caderno" inline no próprio diálogo (título + coleção-pai opcional), reaproveitando `notebook_collection_create` já existente — sem sair do fluxo de salvar. A hierarquia pasta/projeto/caderno/secção já existia (`collections.kind` + `parent_id`); isto só dá acesso a criar um novo nó dela sem abrir a aba Notebooks à parte.

**3) Árvore do Caderno e corpo da nota demoravam a aparecer.** Em modo VPS cada `invoke` passa pelo túnel SSH — a Biblioteca já tinha cache local (`stale-while-revalidate` via `localStorage`) para a lista, mas o Caderno não tinha nenhuma. Adicionado o mesmo padrão: a árvore (coleções+notas) e o corpo de cada nota aberta ficam em cache local por sessão/conta; ao reabrir, o que já se viu aparece imediatamente e o pedido fresco corre por trás, substituindo em silêncio quando chega. Cache isolada por modo+conta (mesma regra da Biblioteca) e limpa em qualquer logout/troca de conta.

**Nota importante para quem testar a seguir:** os prints enviados nesta conversa mostravam a app em **v0.31.0** — antes de todas as fatias 5 a 9 (editor rico, anotações, versões, dicionário, exportação, backlinks) e antes destas três correções. Nada disso aparece até recompilar com `tauri build --no-bundle` a partir do código atual (v0.36.0) e reabrir o `.exe` gerado.

**Validação:** `tsc --noEmit` limpo; teste de fumo cobrindo `insert_document`/releitura via `library_item` (o caminho que `document-save` usa). Build real do Windows continua por fazer.

**Impacto em dados, custo ou privacidade:** a correção do ponto 1 evita desperdício de custo de API já gasto (o objetivo direto da queixa). Sem novo dado sensível — cache local fica só no dispositivo do próprio utilizador, isolada por conta, limpa no logout.

### Registro — 2026-08-11: backlinks entre notas do Caderno

**Contexto:** pesquisa de mercado pedida por Leonardo (Notion/Obsidian/Craft e bibliotecas de editor por blocos como BlockNote/Tiptap/ProseMirror, antes de implementar qualquer coisa nova). Duas conclusões relevantes: (1) o modelo de blocos com ID estável implementado na entrada anterior é, em essência, o mesmo modelo de dados usado pelas libs de mercado (BlockNote é construído sobre Tiptap/ProseMirror, com sincronização Yjs/CRDT só para colaboração em tempo real — que o UpexNote não precisa agora); (2) o que Obsidian/Logseq/Roam têm e o UpexNote ainda não tinha é **bidirectional linking** (ligações entre notas com "backlinks" automáticos) — feature de baixo custo, sem depender de IA.

**O que mudou:** nova tabela `notebooks.note_links` (id, from_note_id, to_note_id, ambos FK para `notebooks.notes`, deleted_at, user_id, host) + `notebooks.note_links_history` para o padrão de snapshot-antes-de-apagar já usado no resto do domínio. Ligação **direcionada, escolhida explicitamente pelo utilizador** (um seletor com as notas existentes) — sem parsing de `[[wikilinks]]` no corpo do texto, o que evitaria ambiguidade de título duplicado e mantém a implementação simples. `notebook_links(note_id)` devolve as duas direções já resolvidas com o título da outra nota: `outgoing` (para onde esta nota aponta) e `incoming` (quem aponta para esta — os "backlinks" propriamente ditos, calculados por leitura inversa, nunca gravados em duplicado).

**Onde aparece:** nova aba "Ligações" no painel lateral do editor de notas, ao lado de Anotações/Referências/Glossário/Exportar/Versões. Clicar num item de qualquer uma das duas listas navega direto para a outra nota (reaproveitando `openNote` já existente na mesma vista — não precisou do mecanismo de âncoras entre vistas usado por `NotebookOpenRequest`). Também virou uma nova camada opcional na exportação/pacote para IA (fatia 8): `links`.

**Validação:** teste de fumo em SQLite local cobrindo criação, listagem nas duas direções, rejeição de auto-ligação e de duplicado, isolamento entre utilizadores, apagar com snapshot no histórico, e inclusão na exportação. `tsc --noEmit` limpo. `Cargo`/build real do Windows continua por fazer (ver nota de sempre no fim desta secção).

**Impacto em dados, custo ou privacidade:** nenhum — sem chamada paga, sem novo dado sensível; só uma relação estruturada entre notas do próprio utilizador.

### Registro — 2026-08-11: modelo de blocos com ID estável no editor do Caderno

**O que mudou:** `notebooks.note_contents.body` deixa de ser um blob único de HTML e passa a guardar uma lista JSON de blocos `[{id, tag, html}]` — cada filho direto do editor (parágrafo, título, lista) carrega um `data-bid` estável, gerado uma vez e preservado enquanto a pessoa continuar a editar (o DOM não recria o nó só por causa de uma tecla). Nenhuma coluna nova: a coluna continua `TEXT`, só muda o que vai dentro dela. Sem dependência nova (continua `contentEditable` nativo — decisão de risco de build já registada).

**Por quê:** era a lacuna explícita apontada nesta sessão — anotações (fatia 6) até então ancoravam contra a nota inteira (`block_id` sempre `null`), então editar um parágrafo acima quebrava a âncora de um comentário abaixo. Agora, quando a seleção cai inteira dentro de um único bloco, a anotação grava o `block_id` real + offset relativo a esse bloco.

**Retrocompatibilidade:** o worker (`_notebook_body_to_text`) e o editor (`nbBodyToEditableHtml`) leem os três formatos que uma nota pode ter: (1) JSON de blocos (novo), (2) blob de HTML solto (notas feitas entre a fatia 5 e esta mudança), (3) texto simples (fatia 3/4 e "Salvar no Caderno"). Formatos antigos ganham blocos com id novo na primeira abertura no editor; nada precisa de migração de schema.

**Limitação aceite:** seleção que atravessa mais de um bloco ainda cai no fallback antigo (`block_id: null`, offset contra a nota inteira) — simplificação por ora.

**Validação:** testado em SQLite local — id estável sobrevive a `notebook_note_version_restore`; anotação grava o `block_id` correto; export/pacote para IA (fatia 8) interpretam os três formatos sem quebrar notas antigas. `tsc --noEmit` limpo.

**Impacto em dados, custo ou privacidade:** nenhum — só formato interno do campo `body`, sem chamada paga.

### Registro — 2026-08-11: direção do chat ancorado (fatia 9) e tiers de acesso por pacote

**Direção e autoria:** decisão de Leonardo Cunha, registada antes da implementação para não se perder até a fatia 9 (`docs/NOTEBOOK_ARCHITECTURE.md`, secção 14) ser retomada. Documentação apenas — nenhum código, schema ou instalador foi alterado nesta entrada.

### O que mudou
- Direção de produto para o **chat ancorado** (fatia 9 do domínio `notebooks`) e para um futuro **controlo de acesso por pacote/plano**, ainda não escritos em nenhum lugar do repositório.

### Decisões — chat ancorado
- **Mesma disciplina da secção 5 (testes de motor de transcrição) aplica-se ao chat**: antes de implementar, testar vários motores/APIs num escopo fechado, comparando custo, tendência a alucinar e capacidade de manter o contexto completo do UpexNote sem "viajar" para fora do assunto. Um modelo "extraordinário" não é necessário — o objetivo aproxima-se de uma automação tipo recepcionista/FAQ (comparável a um bot de WhatsApp bem desenhado), não um assistente genérico caro.
- **Escopo fechado por design**: o agente identifica o problema/dúvida (ex.: pergunta sobre funcionalidade, dúvida de navegação, objeção sobre pacote/acesso) e direciona — nunca responde livremente fora do contexto da aplicação. Nada do que está fora do UpexNote deve vazar para a resposta.
- **Direcionamento por âncoras, não por explicação em texto**: quando a resposta é "onde fica isto", o chat devolve uma âncora que leva a pessoa direto ao lugar certo, em vez de descrever em palavras. Isto reaproveita o padrão já existente no código (`NotebookOpenRequest`/`openInNotebook`, App.tsx — fatia 4): a ideia é generalizar esse mecanismo de "uma ação aponta para um lugar específico da interface" para um roteador de âncoras usado tanto pelo chat quanto por outras partes da app.
- **Custo/acesso à API — duas opções em aberto, decisão adiada para quando os testes de motor da fatia 9 acontecerem:**
  1. *Trial com a chave/custo do próprio UpexNote* (ex.: primeiros N dias ou N mensagens grátis, depois cobra ou pede chave própria) — fricção zero para experimentar, mas exige quota/controlo de custo por utilizador desde o primeiro dia.
  2. *BYOK (bring your own key) desde o início* — o utilizador entra com a própria chave, no mesmo espírito de como os motores de transcrição/formatação já funcionam hoje (`credentials.py` guarda chave por serviço) — custo/risco de abuso zero para o UpexNote, mas cria fricção de entrada.
- Isto conecta com a opção estratégica de usar **n8n como camada de orquestração** já registada na secção 10, item 9 (self-hosted, custo incremental zero pelo n8n em si) — continua sem ser um compromisso, é uma opção a pesar quando a fatia 9 for implementada (n8n vs. chat nativo no worker).

### Decisões — tiers de acesso por pacote
- O perfil do utilizador (o card no canto inferior esquerdo da interface — avatar, username, tag) ganha uma segunda dimensão de tag além de admin/utilizador: uma **tag de plano/pacote** (ex.: Basic, Plus, Premium — nomes ainda não fechados).
- Cada submódulo da aplicação (Caderno, chat ancorado, dashboard, tradução, dicionário/balão de definição sobre palavra, transcrição ao vivo, etc.) passa a ser uma **feature gateável por esse plano**. Este é o motivo explícito, além da organização, de o produto já estar sendo dividido em submódulos/schemas isolados desde a fundação do domínio `notebooks` — a separação existe para permitir restringir por capacidade sem reengenharia depois.
- Racional de produto: soltar funcionalidade aos poucos evita afogar quem está a começar e cria gatilho de upsell explícito ("isto é Premium") — visível ou pela própria interface (ex.: ícone de info explicando o que aquele pacote desbloqueia) ou pelo site.
- Isto conecta com os três cenários de deployment já existentes no `README`/documentação de produto (executável simples / multiacesso para microempresa / VPS empresarial completa): o plano de assinatura passa a ser mais um eixo de decisão sobre o que cada instalação expõe.
- **Nada disto foi modelado em schema ainda.** `users.role` já existe (`admin`/`user`); a extensão para plano/pacote e para feature flags por submódulo fica para quando a fatia 9 (ou uma fatia de acesso dedicada) for desenhada — não deve ser antecipada por estética, mesmo espírito da regra já usada para `notebooks.chat_threads` na secção 9 do `NOTEBOOK_ARCHITECTURE.md`.

### Impacto em dados, custo ou privacidade
- Documentação apenas. Nenhuma API paga foi chamada, nenhum schema foi alterado.

### Próximo passo
- Antes de qualquer código de chat: montar a bateria de testes de motor (mesma metodologia da secção 5), incluindo tentativas deliberadas de fazer o modelo escapar do escopo, para escolher motor(es) por custo/fidelidade ao escopo.
- Decidir trial vs. BYOK antes de desenhar `notebooks.chat_threads`/`chat_messages` (o custo por utilizador só é modelável depois dessa escolha).
- Desenhar o esquema de planos/tags de pacote e feature-gating como uma decisão própria, separada da implementação do chat em si.
