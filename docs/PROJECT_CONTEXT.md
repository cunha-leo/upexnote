...
- **Telemetria privada + token de instalação implementados localmente (2026-07-23):** `POST /v1/tokens/exchange` emite token opaco de 90 dias apenas após opt-in; `POST /v1/telemetry/events` exige esse token e aceita somente ID anónimo (guardado como hash), versão, motor, duração, custo estimado, região e código de erro. O contrato estrito rejeita campos como transcript, áudio, vídeo, caminhos, credenciais ou diagnósticos arbitrários. Persistência Postgres idempotente e 12 testes locais aprovados; integração visual do consentimento na desktop, webhook e publicação no EasyPanel continuam pendentes.
