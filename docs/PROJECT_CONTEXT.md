...
13. **Distribuição real / multi-utilizador (visão do utilizador, 2026-07-16 — não agendado).** Objetivo: enviar o instalador a qualquer pessoa e a app funcionar por completo (instala → abre → usa → persiste), sem dependências de base de dados — só as chaves API próprias. Arquitetura em 2 andares, decidida em discussão:
   - **Andar A — banco embutido (pré-requisito, 1-2 sessões):** SQLite embutido criado no primeiro arranque em `%APPDATA%\UpexNote` (padrão de toda app desktop: Obsidian, WhatsApp Desktop). `db.py` ganha segundo backend + switch no config: instalação do utilizador continua a apontar à VPS (modo power user); qualquer outra usa o embutido. Schema hub-and-spoke porta quase 1:1. NOTA: a rejeição de SQLite de 2026-07-12 era sobre o setup PESSOAL (durabilidade) e mantém-se; para distribuição o cálculo inverte. Sem login neste andar — a sessão do Windows é a identidade.
   - **Andar B — contas + nuvem (comercial, projeto maior):** login na app → **API fina na VPS** (nunca ligação direta de terceiros ao Postgres; possível 1ª fase via n8n, ver item 9) → multi-tenant com tabela `users` + `user_id` no hub — **as FKs/dimensão `actors` reservadas na decisão (c) do item 4 são exatamente este encaixe**. SQLite local vira cache offline com sync (stale-while-revalidate bidirecional). Inclui assinatura digital do instalador e build de distribuição sem `db_config.json` (o `build_worker.ps1` já o suporta).
