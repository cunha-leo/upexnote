...
13. **Distribuição / arquitetura multi-instalação — DECIDIDA em 3 fases (2026-07-16, discussão utilizador+IA; não agendada).** Contexto: a app é hoje extremamente pessoal, pode vir a ser partilhada amigavelmente; venda NÃO está nos planos (fase 3 é ideia futura, nada definido). Mas o utilizador quer levá-la "o mais profissional possível" desde já. Modelo decidido: **conteúdo local + gestão central por eventos** (o conteúdo da pessoa nunca sai da máquina dela; o admin vê métricas, não material).
   ...
   - **Seleção de modo — DECIDIDO (2026-07-16, preferência explícita do utilizador):** ecrã de **login com dois perfis** no arranque, em vez de deteção silenciosa por ficheiros. **Utilizador** → entra direto, SQLite, zero fricção. **Administrador** → formulário cujos campos SÃO os segredos reais (host/porta/user/password da VPS) e cuja validação é uma ligação REAL à VPS — nada de senha embutida no binário (extraível = teatro de segurança); sem os dados exatos, a porta não abre a ninguém. Dentro do ambiente admin, se faltar a chave SSH na máquina (ex.: máquina nova após morte da anterior), a app **gera o par de chaves e guia o único passo manual** (colar a chave pública no painel Hostinger → Chaves SSH → Testar) — o runbook §8 vira assistente visual; o utilizador não quer nunca mais gerir chaves à mão. Raiz de confiança para recuperação total: conta Hostinger (não morre com máquinas). Indicador visível do armazenamento ativo nas Definições ("VPS (Postgres)" vs "local nesta máquina").
